...
def analyze_image_with_openai(image_url, api_token):
  ...
  "website_analysis": {
    ...
    logging.info(f"Analyzing image at {image_url} with GPT-4o-mini")
    try:
...
def process_websites(websites, api_token, run_number, screenshot):
    ...
    for site in websites:
        ...
        try:
            # Use the existing screenshot
            if screenshot is None:
                logging.error(f"No screenshot available for {title}. Skipping to next website.")
                continue
...
if __name__ == "__main__":
    ...
    if websites:
        # Take only one screenshot for all runs
        first_website = websites[0]
        screenshot = take_screenshot(first_website.get('link'))
        if screenshot is None:
            logging.error("Failed to take screenshot. Aborting experiment.")
        else:
            for run in range(1, 8):  # 7 runs
                setup_logging(run)
                logging.info(f"Starting run {run}")
                results = process_websites(websites, api_token, run, screenshot)
