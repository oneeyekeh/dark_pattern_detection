...
# X TBD: Count websites with median darkness > 25
websites_above_25 = df_sorted.groupby('website')['median_darkness'].first()  # First median value for each website
X_TBD = (websites_above_25 > 25).sum()

# Y TBD: Count websites that encountered issues (websites with no valid runs)
total_websites = data['website'].nunique()
websites_with_issues = total_websites - df_sorted['website'].nunique()  # Websites without any valid runs
...
# Identify websites with the highest darkness scores
top_websites = websites_above_25.nlargest(5).index.tolist()  # Get top 5 websites with highest median darkness

# Display TBD results
print(f"\nThe analysis reveals that most websites have an overall darkness score exceeding 20.")
print(f"Specifically, {X_TBD} websites scored above 25, which represents the median value.")
print(f"During the measurement process, {websites_with_issues} websites encountered issues with screenshot capture.")
print(f"The websites with the highest darkness scores are: {', '.join(top_websites)}")
